...
The following are the inputs from Nightingale:
 ...
 - mask inner radius: 0.25
 - mask outer radius: 1.6
 - effective radius: 1.39
 - axis ratio: 0.93 For SIE
 - Position angle: 106.2 (For SIE, See Table 2 of Paper)
 - einstein radius: 1.04 (ONLY for Total mass model)
'''
...
#Could pass priors here to make search go faster
#   Note that some priors will be skipped here, and some of the ones commented out haven't been set to SLACS1 yet
...
disk = af.Model(al.lmp.EllExponential)

# =============================================================================
#bulge.intensity = af.GaussianPrior(mean=4.17, sigma=0.37)
#bulge.sersic_index = af.GaussianPrior(mean=2.36, sigma=0.75)
#bulge.effective_radius = af.GaussianPrior(mean=0.141, sigma=0.035)
...
#disk.intensity = af.GaussianPrior(mean=5.64, sigma=0.25)
...
#SLACS1 light profiles share the same centre and mass-to-light ratio
bulge.centre=disk.centre = (0,0)
bulge.mass_to_light_ratio = disk.mass_to_light_ratio = af.GaussianPrior(mean=5.26,sigma=0.08)
...
source_bulge.centre_0 = af.UniformPrior(lower_limit=-0.2, upper_limit=0.2)
source_bulge.centre_1 = af.UniformPrior(lower_limit=-0.2, upper_limit=0.2)
...
bulge = result_1.model.galaxies.lens.bulge #was getting errors for result_1.model. Maybe should set model/instance parameters directly
