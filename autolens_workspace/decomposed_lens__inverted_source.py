...
#Note that the source hyper-mode model will be specified later, so unecessary to list here I believe
...
initialize the model priors . In this example it:
 ...
 - Uses a parametric `EllSersic` bulge and `EllExponential` disk with centres aligned for the lens galaxy's 
 light and its stellar mass [# parameters: fixed from LIGHT PARAMETRIC PIPELINE].

 - The lens galaxy's dark matter mass distribution is a `SphNFW` whose centre is aligned with bulge of 
 the light and stellar mass model above [# parameters].
